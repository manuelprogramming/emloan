...
def annualized_rate(
    end_capital: Union[float, np.ndarray, pd.Series], starting_capital: float, period: Union[float, np.ndarray, pd.Series]
) -> float:
...
def compound_interest(
    ...
    **kwargs
    ...
    installment_array = installments.create_installment(
        period, installment, installment_type, **kwargs
    )
    installment_interests = np.array(
        [
            installment_array[i] * (1 + interest_rate) ** (period - i)
...
def compound_interest_detailed(
    ...
    **kwargs
    ...
    for period_delta in periods:
        total_net, equity = compound_interest(
        ...
        compound_df = pd.concat(
            [compound_df if not compound_df.empty else None, cur_df], axis=0
        )
